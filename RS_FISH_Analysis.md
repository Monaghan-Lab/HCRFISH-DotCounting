...
- [RS-FISH FIJI plugin to identify
  dots](#rs-fish-fiji-plugin-to-identify-dots)
  - [Setting parameters in interactive
    mode](#setting-parameters-in-interactive-mode)
  - [Dot detection in advanced mode](#dot-detection-in-advanced-mode)
...
Now to dive into the parameters. You can read more about them and what
they do [on the RS-FISH Github
...
regarding the plugin’s use, so you can follow along their tutorial if
you prefer.
...
These are arbitrary starting values, and will likely need to be
adjusted.
...
When the calculations are finished, the `ROI Manager` will open with
your detected dots. Change the point type to `Dot`, and size to `Small`.
The color doesn’t matter, but my default is `white`. Now you can move on
to the next step of making the Maxima image.
...
example,
`10DPA_Distal-DMSO_Cyp26b1-Cyp26a1-Crabp2_Animal5_Whole-Tissue_Crabp2.tif`
would become
`10DPA_Distal-DMSO_Cyp26b1-Cyp26a1-Crabp2_Whole-Tissue_Crabp2_Maxima.tif`.
